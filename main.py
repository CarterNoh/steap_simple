...
def createFactorGraph(robot_model, sdf, start, end):
    # init optimization
    graph = NonlinearFactorGraph()
    init_values = Values()
    start_pos, start_vel = start
    end_pos, end_vel = end
    ...
    for i in range(0, total_time_step + 1):
        ...
        pose = start_pos * float(total_time_step - i) / float(
            total_time_step) + end_pos * i / float(total_time_step)
        vel = avg_vel
        print(pose)
        ...
        # start/end priors
        if i == 0:
            graph.push_back(PriorFactorVector(key_pos, start_pos, pose_fix))
            graph.push_back(PriorFactorVector(key_vel, start_vel, vel_fix))
        elif i == total_time_step:
            graph.push_back(PriorFactorVector(key_pos, end_pos, pose_fix))
        ...
        if i > 0:
            ...
            # cost factor
            graph.push_back(
                ObstaclePlanarSDFFactorPointRobot(key_pos, robot_model, sdf,
                ...
                for j in range(1, check_inter + 1):
                    ...
                    graph.add(
                        ObstaclePlanarSDFFactorGPPointRobot(key_pos1, key_vel1, key_pos2, key_vel2,
                            robot_model, sdf, cost_sigma, epsilon_dist, Qc_model, delta_t, tau))
...
end_pos = np.asarray([17, 14])
end_vel = np.asarray([0, 0])
avg_vel = (end_pos / total_time_step) / delta_t
...
for i in range(total_time_step + 1):
    ...
    plotPointRobot2D(figure, axis, pR_model, pos)
